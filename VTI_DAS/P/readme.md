# P: fixed-event qP-only VTI Bayesian inversion

## Purpose of this package

This folder is designed for qP-only VTI parameter inversion with fixed microseismic event locations. The event coordinates are kept at the synthetic true locations, and the inversion updates only the layered VTI model parameters and interface depths. This setting is useful for testing how much information is provided by qP adjacent-channel DAS differential traveltimes and for diagnosing qP-only parameter nonuniqueness.

This experiment corresponds to the fixed-event qP-only calibration test in the manuscript. The event locations are not part of the inversion, so the main focus is the posterior constraint on `alpha0`, `beta0`, `epsilon`, `gamma`, `delta`, and interface depths.
...
- only qP traveltimes are used;
- adjacent-channel qP differential traveltimes are used in the likelihood;
- event locations are not inverted;
- `04-initial/output/geo.dat` is copied from `01-input/output/geometry.dat`, so the initial event locations are exactly the true event locations;
- interface depths are inverted;
- the last half-space row of the velocity model is fixed.

The grid-search parameters such as `INIT_GRID_X_MIN`, `INIT_GRID_X_MAX`, `INIT_GRID_Z_MIN`, and `INIT_GRID_Z_MAX` are not used in the default setting because `FIX_SOURCE_TO_TRUE=True` bypasses event-location initialization. They are kept only for possible future location tests.
...
## Current qP-only inversion variables

With the default configuration, the MCMC parameter vector mainly contains:
...
Because `FIX_LAST_LAYER=True`, the bottom half-space row is not updated. Because `INVERT_SOURCES=False`, `sx` and `sz` are not included in the MCMC parameter vector.
...
## Notes specific to the qP-only fixed-event experiment

1. qP differential data directly constrain `alpha0` and some qP-sensitive anisotropy combinations.
2. Parameters that are mainly related to S-wave behaviour, especially `beta0` and `gamma`, are usually less unique in a qP-only objective.
3. Because event locations are fixed, the location-velocity trade-off is artificially removed. This package is therefore best used as a VTI parameter identifiability test.
4. To run a joint event-location and VTI inversion, use the `P_event_VTI` package rather than only changing `INVERT_SOURCES`. The joint-location package already contains the event grid-search initialization and event-parameter update logic.
...
- To change the noise level, modify only `OBS_NOISE_STD` in `shell_simple.py`.
- To change the MCMC length, modify `MCMC_ITERATIONS` and `MCMC_BURNIN`.
- To run a three-phase fixed-event test, use the `P_SV_SH` package.
- To change the true model or DAS geometry, edit event coordinates, DAS coordinates, and VTI parameters in `01-input/direct.py`.
- To change prior widths, modify `PRIOR_*_MAX_DEV` and `PRIOR_*_MARGIN`.
- To tune MCMC moves, modify the `PROP_*_STD` parameters.
