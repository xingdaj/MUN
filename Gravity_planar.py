...
class PlanarFlow(nn.Module):
    ...
    def inverse(self, z):
        """Inverse transformation and log determinant"""
...
# ================== Normalizing Flow with Planar Layers ==================
class NormalizingFlow(nn.Module):
    """Normalizing flow using planar transformations"""
    def __init__(self, dim=3, num_flows=20):
        super().__init__()
        self.dim = dim
        ...
        # Create planar flow layers with leaky_relu activation
        self.flows = nn.ModuleList([
            PlanarFlow(shape=(dim,), act="leaky_relu")
    ...
    def forward(self, n_samples=1):
        ...
        # Apply all planar transformations
    ...
    def log_prob(self, z):
        ...
        # Apply inverse planar transformations in reverse order
...
# ================== ELBO Calculation ==================
def compute_elbo(model, y, xx, sigma_noise=0.5, n_samples=32):
...
# ================== Training Loop ==================
model = NormalizingFlow(dim=3, num_flows=20)  # Now dim=3
...
for epoch in range(num_epochs):
    ...
    elbo = compute_elbo(model, y_tensor, xx_tensor)
    (-elbo).backward()
