...
Purpose
-------
This file condenses the MATLAB 05-inversion workflow into one Python script:
...
Default input/output layout follows the MATLAB folder style:
    input_dir  = ../04-initial
        geo.dat      geometry
        vel.dat      initial layered VTI model
        nobs.dat     observed qP/qSV/qSH travel times
        prior.dat    parameter bounds
        prop.dat     proposal standard deviations

For test 1, use --no-invert-sources with --objective diff-p-adjacent and
--use-waves P,SV,SH to fix sources while constraining VTI parameters with
three-phase station-pair differential data.
    output_dir = ../06-result
        mean.dat, best.dat, chain.npz, misfit.png

Notes
-----
1. This version keeps the number of layers fixed, but updates layer-interface
   depths by default. It inverts:
       dep[1], dep[2], ..., dep[nlayer-1]
...
DEFAULT_STOP = 1.0e-6  # fallback only; main() reads ../01-input/output/control.dat when --qx-stop is not given
DEFAULT_MAX_ITER = 20
...
def resolve_qx_settings(qx_stop_arg: float | None, qx_max_iter_arg: int | None) -> tuple[float, int]:
    ...
    01-input/output/control.dat had been changed to 1e-6.  This function makes
    MCMC read the same control.dat used by vti_direct.py unless the user
    explicitly provides --qx-stop/--qx-max-iter.
    """
    script_dir = Path(__file__).resolve().parent
    candidates = [
        script_dir.parent / "01-input" / "output" / "control.dat",
        Path("../01-input/output/control.dat"),
        Path("../01-input/control.dat"),
    ]
...
def read_observed(path: str | Path) -> ObsData:
    ...
    The sx/sz/rx/rz columns are kept so they can be checked against geo.dat.
    They are NOT used to define the inversion geometry; forward modelling still
    uses geo.dat.  This prevents silent inversion with mismatched observation
    and modelling geometries.
    """
    vals = float_tokens(path)
    if len(vals) < 3:
    ...
    if len(vals) < expected:
        raise ValueError(
            f"Observed file {path} is incomplete: expected at least {expected} numeric values "
            f"for ns={ns}, nr={nr}, got {len(vals)}"
        )
    ...
    for isrc in range(ns):
        for ir in range(nr):
            # MATLAB nobs.dat rows: sx sz rx rz tp tsv tsh
...
def validate_observed_geometry(geo: Geometry, obs: ObsData, tol: float = 1e-6,
    ...
    """Validate geometry embedded in nobs.dat.

    Important convention:
    - Receiver geometry must match geo.dat, because receivers are fixed in the
      forward model and define the station-pair differences.
    - Source geometry in nobs.dat is usually the TRUE source geometry used to
      generate synthetic observations, whereas geo.dat often stores the INITIAL
      source guess that MCMC will update.  Therefore source-vs-geo.dat checking
      is NOT fatal by default.  Use --check-nobs-source-geometry only when
      geo.dat is expected to contain the exact observation source locations.
    """
    ...
    if obs.sx_row is None or obs.sz_row is None or obs.rx_row is None or obs.rz_row is None:
        raise ValueError("nobs.dat geometry columns were not loaded; cannot validate observation geometry")

    tol = float(tol)
    if tol < 0.0 or not math.isfinite(tol):
        raise ValueError(f"Invalid geometry tolerance: {tol!r}")

    # Receiver coordinates should repeat identically for every source.
    ...
    # nobs.dat source coordinates should be internally consistent across receivers
    # for each source, even when they intentionally differ from the initial source
    # locations in geo.dat.
...
def read_prior(path: str | Path) -> Prior:
    vals = float_tokens(path)
    if len(vals) < 29:
        raise ValueError(f"prior.dat should contain at least 29 numeric values, got {len(vals)}")
    return Prior(
        ...
        tmin=vals[26], tmax=vals[27], dt=vals[28],
    )
...
def forward_direct_source(geo: Geometry, model: VTIModel, isrc: int,
    ...
    for ir in range(geo.nr):
        ...
        if not np.any(Z > 0.0):
            z_mid = 0.5 * (sz + rz)
            if "P" in needed:
                tp_row[ir] = horizontal_direct_result(1, H, z_mid, model, max_iter)["ttime"]
            if "SV" in needed:
                tsv_row[ir] = horizontal_direct_result(2, H, z_mid, model, max_iter)["ttime"]
            if "SH" in needed:
                tsh_row[ir] = horizontal_direct_result(3, H, z_mid, model, max_iter)["ttime"]
            continue

        fail_tol = qx_acceptance_tolerance(stop)
        if "P" in needed:
            _, px1, err1, _, _, conv1, _ = solve_qx(1, H, Z, model, stop, max_iter, pmax_cache=pmax_cache[1])
            if abs(err1) > fail_tol:
                raise FloatingPointError(
        ...
        if "SV" in needed:
            _, px2, err2, _, _, conv2, _ = solve_qx(2, H, Z, model, stop, max_iter, pmax_cache=pmax_cache[2])
            if abs(err2) > fail_tol:
                raise FloatingPointError(
        ...
        if "SH" in needed:
            _, px3, err3, _, _, conv3, _ = solve_qx(3, H, Z, model, stop, max_iter, pmax_cache=pmax_cache[3])
            if abs(err3) > fail_tol:
                raise FloatingPointError(
...
def active_depth_indices(model: VTIModel) -> np.ndarray:
    """Layer-interface depths to invert.

    dep[0] is the model top and is kept fixed. For an n-layer model, dep[1]
    through dep[nlayer-1] are the interfaces between layers, including the top
    of the bottom half-space.
    """
    return np.arange(1, model.nlayer, dtype=int)


def pack_params(geo: Geometry, model: VTIModel, invert_depths: bool, fix_last_layer: bool,
                invert_sources: bool = True) -> np.ndarray:
    layers = active_layer_indices(model, fix_last_layer)
    parts = []
    if invert_depths:
        # dep[0] is the model top and is fixed; update all layer interfaces.
...
def unpack_params(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
                  ...
                  invert_sources: bool = True) -> Tuple[Geometry, VTIModel]:
...
def prior_violation_messages(geo: Geometry, model: VTIModel, initial_model: VTIModel, prior: Prior,
                             invert_depths: bool, fix_last_layer: bool,
                             invert_sources: bool = True) -> list[str]:
...
def print_initial_objective_diagnostics(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
    ...
    geo, model = unpack_params(theta, template_geo, template_model, invert_depths, fix_last_layer, invert_sources)

    msgs = prior_violation_messages(geo, model, initial_model, prior, invert_depths, fix_last_layer, invert_sources)
    if msgs:
    ...
    if not math.isfinite(mis):
        ...
        if like.objective_type in {"diff-p-adjacent", "diff-p-reference"} and like.diff_matrix is not None and like.obs_d is not None:
            pred_map = {"P": tp, "SV": tsv, "SH": tsh}
            obs_d = np.asarray(like.obs_d, dtype=float)
            if obs_d.ndim == 2:
                obs_d = obs_d[None, :, :]
            for iw, wave in enumerate(like.waves):
                pred_d = apply_difference(pred_map[wave], like.diff_matrix)
                res = pred_d - obs_d[iw]
                print(
                    f"[INIT-CHECK][RESIDUAL] diff-{wave} finite={np.all(np.isfinite(res))}, "
...
def difference_matrix(nr: int, mode: str) -> np.ndarray:
    ...
    mode="adjacent":  d_i = t_{i+1} - t_i, i=0,...,nr-2.
    mode="reference": d_i = t_{i+1} - t_0, i=0,...,nr-2.

    Both choices produce nr-1 independent differences and remove the unknown
    source origin time.  They have different covariance signs:
      adjacent  -> offdiag = -sigma^2
      reference -> offdiag = +sigma^2
    """
    nr = int(nr)
    if nr < 2:
...
def build_likelihood_config(obs: ObsData, objective_type: str, use_waves: str,
    ...
    if objective_type in {"diff-p-adjacent", "diff-p-reference"}:
        ...
        obs_map = {"P": obs.tp, "SV": obs.tsv, "SH": obs.tsh}
        # Shape: (nwave, ns, npair).  Each wave uses the same station-pair
        # differencing matrix and covariance, and wave-picking noises are treated
        # as independent between qP/qSV/qSH.
        obs_d = np.stack([apply_difference(obs_map[w], A) for w in waves], axis=0)
        ...
        return LikelihoodConfig(
            ...
            chol=L,
            obs_d=obs_d,
            ndata_per_source=A.shape[0] * len(waves),
        )
...
def misfit_from_times(pred, obs: ObsData, like: LikelihoodConfig) -> float:
    ...
    For absolute arrivals, C = sigma^2 I.
    For differential arrivals, d = A t and C_d = sigma^2 A A^T for each selected wave.
    The log-determinant term is omitted because sigma and A are fixed during
    MCMC, so it cancels in the Metropolis ratio.
    """
    ...
    if like.objective_type == "absolute":
        ...
        waves = like.waves
        if "P" in waves:
            mis += float(np.sum(((tp - obs.tp) / sigma) ** 2))
        if "SV" in waves:
            mis += float(np.sum(((tsv - obs.tsv) / sigma) ** 2))
        if "SH" in waves:
            mis += float(np.sum(((tsh - obs.tsh) / sigma) ** 2))
        return mis
    ...
    if like.objective_type in {"diff-p-adjacent", "diff-p-reference"}:
        ...
        obs_d = np.asarray(like.obs_d, dtype=float)
        if obs_d.ndim == 2:
            # Backward compatibility with older qP-only chain files/configs.
            obs_d = obs_d[None, :, :]
        if obs_d.shape[0] != len(like.waves):
            return math.inf
        mis = 0.0
        for iw, wave in enumerate(like.waves):
            pred_d = apply_difference(pred_map[wave], like.diff_matrix)
            r = pred_d - obs_d[iw]
            mis += quadratic_form_inverse(r, like.cov_inv)
        return mis
...
def likelihood_covariance_diagnostics(obs: ObsData, like: LikelihoodConfig) -> str:
    ...
    if objective_type in {"diff-p-adjacent", "diff-p-reference"}:
        ...
        waves_text = "/".join("q" + w if w != "P" else "qP" for w in like.waves)
        ndata = ns * A.shape[0] * max(1, len(like.waves))
        return (
            f"[LIKELIHOOD] objective={objective_type}, waves={waves_text}, {mode} differential arrivals, sigma={sigma:.6g}, sigma_mode={like.sigma_mode}\n"
            f"[LIKELIHOOD] convention: sigma_mode=absolute gives C_d=sigma^2*A*A^T per wave; sigma_mode=objective-iid gives C_d=sigma^2*I per wave\n"
            f"[LIKELIHOOD] per-source A shape={A.shape}, covariance shape={cov.shape}, waves={like.waves}, ndata={ndata}\n"
...
def objective(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
              initial_model: VTIModel, prior: Prior, obs: ObsData,
              invert_depths: bool, fix_last_layer: bool, invert_sources: bool,
              qx_stop: float, qx_max_iter: int,
              like: LikelihoodConfig) -> Tuple[float, Tuple[np.ndarray, np.ndarray, np.ndarray] | None]:
    geo, model = unpack_params(theta, template_geo, template_model, invert_depths, fix_last_layer, invert_sources)
    if not prior_ok(geo, model, initial_model, prior, invert_depths, fix_last_layer, invert_sources):
        return math.inf, None
    try:
        pred = forward_direct(geo, model, stop=qx_stop, max_iter=qx_max_iter, needed_waves=like.waves)
        mis = misfit_from_times(pred, obs, like)
...
def objective_source_update(theta: np.ndarray,
    ...
    The VTI model and all other sources are unchanged, so reuse the current
    full prediction and recompute only one source row.  This preserves the same
    likelihood value as a full forward run while avoiding ns-times extra work
    for source-coordinate scalar updates.
    """
    geo, model = unpack_params(theta, template_geo, template_model, invert_depths, fix_last_layer, invert_sources)
    if not prior_ok(geo, model, initial_model, prior, invert_depths, fix_last_layer, invert_sources):
        return math.inf, None
    try:
        ...
        pmax_cache = _wave_pmax_cache(model, needed)
        row_pred = forward_direct_source(
...
def run_mcmc(input_dir: Path, output_dir: Path, iterations: int, burnin: int,
             ...
             dr_scale: float, update_order: str,
    ...
    print("[VERSION] vti_joint_mcmc_dram.py test1-capable: fixed-source option + 3-wave station-pair differential likelihood", flush=True)
    ...
    theta = pack_params(geo0, model0, invert_depths=invert_depths, fix_last_layer=fix_last_layer, invert_sources=invert_sources)
    base_std = proposal_std_vector(model0, geo0, prop, invert_depths=invert_depths, fix_last_layer=fix_last_layer, invert_sources=invert_sources)
    scalar_std = base_std.copy()
    ndim = theta.size
    names = param_names(model0, geo0, invert_depths, fix_last_layer, invert_sources)
    ...
    mis, pred = objective(theta, geo0, model0, model0, prior, obs, invert_depths, fix_last_layer, invert_sources,
                          qx_stop, qx_max_iter, like)
    if not math.isfinite(mis):
        print_initial_objective_diagnostics(
            theta, geo0, model0, model0, prior, obs,
            invert_depths, fix_last_layer, invert_sources, qx_stop, qx_max_iter, like,
        )
    ...
    for it in range(iterations):
        ...
        if src_update_index is not None and pred is not None:
            mis1, pred1 = objective_source_update(
                theta1, pred, src_update_index, geo0, model0, model0, prior, obs,
                invert_depths, fix_last_layer, invert_sources, qx_stop, qx_max_iter, like,
            )
        else:
            mis1, pred1 = objective(theta1, geo0, model0, model0, prior, obs,
                                    invert_depths, fix_last_layer, invert_sources,
                                    qx_stop, qx_max_iter, like)
        ...
        if math.isfinite(log_alpha1) and math.log(rng.random()) < min(0.0, log_alpha1):
            ...
            if src_update_index is not None and pred is not None:
                mis2, pred2 = objective_source_update(
                    theta2, pred, src_update_index, geo0, model0, model0, prior, obs,
                    invert_depths, fix_last_layer, invert_sources, qx_stop, qx_max_iter, like,
                )
            else:
                mis2, pred2 = objective(theta2, geo0, model0, model0, prior, obs,
                                        invert_depths, fix_last_layer, invert_sources,
                                        qx_stop, qx_max_iter, like)
        ...
        if print_every > 0 and (it + 1) % print_every == 0:
            ...
            print(
                ...
                f"acc={acc_rate:.2f}%, block_time={block_elapsed:.2f}s",
                flush=True,
            )
    ...
    mean_geo, mean_model = unpack_params(mean_theta, geo0, model0, invert_depths, fix_last_layer, invert_sources)
    best_geo, best_model = unpack_params(best_theta, geo0, model0, invert_depths, fix_last_layer, invert_sources)
...
def param_names(model: VTIModel, geo: Geometry, invert_depths: bool, fix_last_layer: bool, invert_sources: bool = True) -> list[str]:
    layers = active_layer_indices(model, fix_last_layer)
    names = []
    if invert_depths:
        depth_idx = active_depth_indices(model)
...
def parse_args(argv=None):
    ...
    p.add_argument("--input-dir", default="../04-initial/output", type=Path)
    p.add_argument("--output-dir", default="../06-result/output", type=Path)
    ...
    p.add_argument("--use-waves", default="P,SV,SH", help="Combination of P, SV, SH used by absolute and differential objectives")
    ...
    p.add_argument(
        ...
        help="Likelihood target. diff-p-adjacent uses t[i+1]-t[i]; diff-p-reference uses t[i+1]-t[0]. Differential objectives apply this to all --use-waves phases and use the induced non-diagonal covariance per phase.",
    )
    p.add_argument("--qx-stop", type=float, default=None,
                   help="qx solver stop tolerance. Default: read ../01-input/output/control.dat, fallback DEFAULT_STOP.")
    p.add_argument("--qx-max-iter", type=int, default=None,
                   help="qx solver max iterations. Default: read ../01-input/output/control.dat, fallback DEFAULT_MAX_ITER.")
    depth_group = p.add_mutually_exclusive_group()
    depth_group.add_argument(
        "--invert-depths",
        dest="invert_depths",
        action="store_true",
        help="Invert layer-interface depths dep[1:nlayer] (default)",
    )
    depth_group.add_argument(
        "--no-invert-depths",
        dest="invert_depths",
    ...
    source_group.add_argument("--invert-sources", dest="invert_sources", action="store_true", help="Invert source sx/sz coordinates (default)")
    source_group.add_argument("--no-invert-sources", dest="invert_sources", action="store_false", help="Keep source sx/sz fixed at geo.dat values")
    p.set_defaults(invert_sources=True)
    ...
    p.add_argument("--update-order", choices=["cyclic", "random"], default="cyclic", help="Choose one scalar parameter cyclically or randomly each iteration")
...
def main(argv=None) -> int:
    ...
    run_mcmc(
        ...
        invert_sources=args.invert_sources,
