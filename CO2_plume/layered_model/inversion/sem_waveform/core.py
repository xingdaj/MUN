...
from .velocity import build_velocity_on_sem_nodes, build_velocity_layered_with_anomaly, build_layered_background
...
class SEMSimulation:
 ...
 #print("==== Velocity Parameters ====")
        ...
        if self.ctrl_pts_original is not None:
            if self.true_offset is not None:
                ctrl6 = (self.ctrl_pts_original + self.true_offset).reshape(-1, 2)
            else:
            ...
            velocity_model_init, signed_dist_init, _extras = build_velocity_layered_with_anomaly(
                nodes_xy=self.global_coords,
                interfaces_z=np.asarray(self.config['velocity'].get('layers', {}).get('interfaces_z', []), dtype=np.float64),
                layer_velocities=np.asarray(self.config['velocity'].get('layers', {}).get('velocities', []), dtype=np.float64),
                ctrl6_xy=ctrl6,
    ...
    def run(self, ctrl_offsets_flat=None, use_torch=True):
        ...
        layers_cfg = vel_cfg.get('layers', {})
        interfaces_z = np.asarray(layers_cfg.get('interfaces_z', []), dtype=np.float64)
        layer_velocities = np.asarray(layers_cfg.get('velocities', []), dtype=np.float64)

        anom_cfg = vel_cfg.get('anomaly', {})
        v_inside = float(anom_cfg.get('v_inside', float(self.vmin)))
        blend = str(anom_cfg.get('blend', 'smooth'))

        # cache layered background on SEM nodes (used by gradient projection)
        self.v_layer_background = build_layered_background(self.global_coords, interfaces_z, layer_velocities)

        self.velocity_model, self.signed_dist, self.spline_extras = build_velocity_layered_with_anomaly(
            nodes_xy=self.global_coords,          # (N,2)
            interfaces_z=interfaces_z,
    ...
    def _analytical_gradient_projection_improved(
        ...
        # For layered background: v = v_layer + (v_inside - v_layer) * sigmoid(-sd/tau)
        v_layer = getattr(self, 'v_layer_background', None)
        if v_layer is None:
            v_layer = build_layered_background(self.global_coords, interfaces_z, layer_velocities)
        v_layer = np.asarray(v_layer, dtype=np.float64).reshape(-1)

        dv_ddsoft = (v_inside - v_layer) * sig * (1.0 - sig) * (-1.0 / tau_safe)

        # Keep only effective contributions
        mask = np.abs(dv_ddsoft) > 1e-8 * np.maximum(1.0, np.abs(v_inside - v_layer))
        if not np.any(mask):
            return {
    ...
    def _finite_difference_gradient_projection(self, perturbations, dJ_dv,
        ...
        for j in range(n_param):
            ...
            v_plus, _, _ = build_velocity_layered_with_anomaly(
                nodes_xy=self.global_coords,
                interfaces_z=interfaces_z,
                layer_velocities=layer_velocities,
                ctrl6_xy=ctrl_plus,
            ...
            v_minus, _, _ = build_velocity_layered_with_anomaly(
                nodes_xy=self.global_coords,
                interfaces_z=interfaces_z,
                layer_velocities=layer_velocities,
                ctrl6_xy=ctrl_minus,
    ...
    def run_forward_and_adjoint(self, params_dict):
        ...
        velocity_model, signed_dist, spline_extras = build_velocity_layered_with_anomaly(
            nodes_xy=self.global_coords,
            interfaces_z=interfaces_z,
            layer_velocities=layer_velocities,
            ctrl6_xy=ctrl6,
        ...
        u_hist = torch.zeros((self.nt, self.npoints), dtype=hist_dtype)
        ...
        for it in range(self.nt):
            ...
            u_hist[it, :] = u_curr.to(hist_dtype)
    ...
    def run_forward_only_for_loglik(self, ctrl12, sigma2, obs_dt, obs_nt, y_obs):
        ...
        **will not** self.velocity_model / self.signed_dist / self.spline_extras.
        ...
        v_local, sd_local, ex_local = build_velocity_layered_with_anomaly(
            nodes_xy=self.global_coords,
            interfaces_z=interfaces_z,
            layer_velocities=layer_velocities,
            ctrl6_xy=perts,
