
import numpy as np
from scipy.interpolate import BSpline, RegularGridInterpolator
from matplotlib.path import Path

# ----------------------------------------------------------------------------------------------------------------------
# Utility: Construct closed B-spline (control points + replicate first degree control points; open interval end knots)
# ----------------------------------------------------------------------------------------------------------------------

def _closed_bspline_from_ctrl(ctrl6_xy: np.ndarray, degree: int = 2):
    """
    Construct the same closed periodic quadratic B-spline used by the model
    generators and by RQS_NF.py plotting.

    Convention:
        ctrl_ext = [P0, P1, ..., P{K-1}, P0, P1, ..., P{k-1}]
        knots    = 0, 1, 2, ..., len(ctrl_ext) + k
        valid parameter interval = [k, K+k)

    This is intentionally NOT the clamped [0,1] knot convention.  The latter
    was the source of a mismatch between the displayed boundary and the SEM
    forward model.
    """
    ctrl6_xy = np.asarray(ctrl6_xy, dtype=np.float64).reshape(-1, 2)
    k = int(degree)
    if ctrl6_xy.shape[0] < k + 1:
        raise ValueError(f"At least {k + 1} control points are required for degree {k}; got {ctrl6_xy.shape[0]}")

    ctrl_closed = np.vstack([ctrl6_xy, ctrl6_xy[:k]])
    knots = np.arange(0.0, len(ctrl_closed) + k + 1, dtype=np.float64)

    spline = BSpline(knots, ctrl_closed, k, extrapolate=False)
    spline_d1 = spline.derivative(1)
    spline_d2 = spline.derivative(2)
    return spline, spline_d1, spline_d2, knots, ctrl_closed


def closed_bspline_curve_points(ctrl_xy: np.ndarray, samples: int = 800, degree: int = 2, close: bool = True):
    """Return curve points using the exact same convention as the SEM model."""
    ctrl_xy = np.asarray(ctrl_xy, dtype=np.float64).reshape(-1, 2)
    k = int(degree)
    spline, _, _, knots, _ = _closed_bspline_from_ctrl(ctrl_xy, degree=k)
    u_min = float(knots[k])
    u_max = float(knots[-(k + 1)])
    u = np.linspace(u_min, u_max, int(samples), endpoint=False, dtype=np.float64)
    curve = np.asarray(spline(u), dtype=np.float64)
    if close:
        curve = np.vstack([curve, curve[:1]])
    return curve

# ----------------------------------------------------------------
# Core: Newton signed distance to true spline (vectorized+chunked)
# ----------------------------------------------------------------
def signed_distance_to_spline_newton(
    coords_xy: np.ndarray,     # (N,2)
    ctrl6_xy: np.ndarray,      # (6,2)
    samples: int = 800,        # initial sampling number (suggest 400~1200)
    newton_steps: int = 7,     # Newton steps per initial guess (5~8 is enough)
    topk: int = 1,             # select top-k initial values per point (1 is enough, can set 2 for more stability)
    chunk: int = 20000,        # chunked to avoid memory peaks
    return_extra: bool = True  # whether to return u_star/closest/tangent/normal
):
    """
    return:
      signed_dist: (N,)
      if return_extra=True, also return:
      u_star: (N,), closest: (N,2), tangent: (N,2), normal_out: (N,2)
    """
    coords_xy = np.asarray(coords_xy, dtype=np.float64)
    N = coords_xy.shape[0]

    # construct closed spline and derivatives
    spline, s1, s2, knots, ctrl_closed = _closed_bspline_from_ctrl(ctrl6_xy, degree=2)

    # uniformly spaced initial values over the periodic valid interval [k, K+k).
    # endpoint=False avoids duplicating the closure point.
    k_degree = 2
    u_min = float(knots[k_degree])
    u_max = float(knots[-(k_degree + 1)])
    u_max_safe = np.nextafter(u_max, u_min)
    u0_grid = np.linspace(u_min, u_max, samples, endpoint=False, dtype=np.float64)
    C_u0 = spline(u0_grid)  # (S,2)

    # for inside/outside sign: use dense polygon sampling inside/outside test (robust and consistent with original logic)
    poly_path = Path(C_u0, closed=True)

    # result buffer
    signed_dist = np.empty(N, dtype=np.float64)
    u_star      = np.empty(N, dtype=np.float64)
    closest     = np.empty((N, 2), dtype=np.float64)
    tangent     = np.empty((N, 2), dtype=np.float64)
    normal_out  = np.empty((N, 2), dtype=np.float64)

    # chunked processing
    for s in range(0, N, chunk):
        e = min(N, s + chunk)
        X = coords_xy[s:e]  # (M,2)
        M = X.shape[0]

        # 1) select initial value: use nearest sampled point as candidate (can select top-k)
        diff = X[:, None, :] - C_u0[None, :, :]          # (M,S,2)
        dist2 = np.sum(diff * diff, axis=2)              # (M,S)
        if topk == 1:
            idx0 = np.argmin(dist2, axis=1)             # (M,)
            U0 = u0_grid[idx0][:, None]                 # (M,1)
        else:
            idx0 = np.argpartition(dist2, topk-1, axis=1)[:, :topk]  # (M,topk)
            U0 = u0_grid[idx0]                                       # (M,topk)

        # 2) Newton refinement
        u = U0.copy()
        for _ in range(newton_steps):
            Cu  = spline(u)    # (M,*,2)
            Cp  = s1(u)
            Cpp = s2(u)
            r   = Cu - X[:, None, :]
            phi1 = np.sum(r * Cp, axis=2)             # (M,*)
            phi2 = np.sum(Cp * Cp, axis=2) + np.sum(r * Cpp, axis=2)
            phi2 = np.where(np.abs(phi2) < 1e-14, 1e-14, phi2)
            du = phi1 / phi2
            u = u - du
            u = np.clip(u, u_min, u_max_safe)

        # 3) select best initial value (if topk>1)
        if topk > 1:
            Cu  = spline(u)                    # (M,topk,2)
            r   = Cu - X[:, None, :]
            dist2_refined = np.sum(r * r, axis=2)  # (M,topk)
            best = np.argmin(dist2_refined, axis=1)
            u_best = u[np.arange(M), best]
        else:
            u_best = u[:, 0]

        # 4) final values: closest point, tangent, normal, distance
        Cb  = spline(u_best)           # (M,2)
        Tb  = s1(u_best)               # (M,2)
        Tb_norm = np.linalg.norm(Tb, axis=1, keepdims=True) + 1e-15
        t_hat = Tb / Tb_norm           # unit tangent (M,2)

        rvec = X - Cb
        dist = np.linalg.norm(rvec, axis=1)

        # 5) sign via inside test (negative inside)
        inside = poly_path.contains_points(X, radius=1e-12)
        sign = np.where(inside, -1.0, 1.0)
        sdist = sign * dist

        # outward normal aligned with sign
        n_geom = rvec / (dist[:, None] + 1e-15)
        n_out  = sign[:, None] * n_geom

        # write back
        signed_dist[s:e] = sdist
        # Store normalized periodic parameter in [0,1).  The gradient code maps
        # it back to the same [k, K+k) B-spline interval.
        u_star[s:e]      = (u_best - u_min) / max(u_max - u_min, 1.0e-300)
        closest[s:e, :]  = Cb
        tangent[s:e, :]  = t_hat
        normal_out[s:e,:]= n_out

    if return_extra:
        return signed_dist, u_star, closest, tangent, normal_out
    else:
        return signed_dist

# -----------------------------------------------------------------
# Original API: Build velocity by single inside/outside values across a spline
# -----------------------------------------------------------------
def build_velocity_on_sem_nodes(
    nodes_xy: np.ndarray,  # (N,2)
    ctrl6_xy: np.ndarray,  # (6,2)
    v_inside: float, v_outside: float,
    tau: float,
    samples: int = 800,
    newton_steps: int = 7
):
    """
    Based on the signed distance to the "true spline closest point" for smooth velocity transition.
    keep consistent with old API: return velocity(N,) and signed_dist(N,)
    Also return (u_star, closest, tangent, normal_out) for ADJ use.
    """
    nodes_xy = np.asarray(nodes_xy)
    if nodes_xy.ndim == 1:
        if nodes_xy.size % 2 == 0:
            nodes_xy = nodes_xy.reshape(-1, 2)
        else:
            raise ValueError(f"'nodes_xy' must be (N,2); got 1D of size {nodes_xy.size}.")
    if nodes_xy.ndim != 2 or nodes_xy.shape[1] != 2:
        raise ValueError(f"'nodes_xy' must be (N,2); got shape {nodes_xy.shape}")
    ctrl6_xy = np.asarray(ctrl6_xy, dtype=np.float64).reshape(-1, 2)

    sd, u_star, Cclosest, t_hat, n_out = signed_distance_to_spline_newton(
        coords_xy=nodes_xy, ctrl6_xy=ctrl6_xy,
        samples=samples, newton_steps=newton_steps,
        topk=1, return_extra=True
    )

    # smooth interface: v = vmax + (vmin-vmax) * sigmoid(-sd/tau)
    tau_safe = max(float(tau), 1e-12)
    x = np.clip(sd / tau_safe, -50.0, 50.0)      # note: sd/tau
    sig = 1.0 / (1.0 + np.exp(x))                # = σ(-sd/τ)
    v = v_outside + (v_inside - v_outside) * sig

    return v, sd, {'u_star': u_star, 'closest': Cclosest,
                   'tangent': t_hat, 'normal_out': n_out}



# -----------------------------------------------------------------
# 2D smooth background (grid) + optional spline anomaly
# -----------------------------------------------------------------
def build_background_from_grid(
    nodes_xy: np.ndarray,   # (N,2)
    xg: np.ndarray,         # (nx,)
    zg: np.ndarray,         # (nz,)
    vp_grid: np.ndarray,    # (nz,nx)  (z-first, x-second)
    method: str = "linear",
    clip: bool = True
):
    """
    Interpolate a 2D background velocity grid onto SEM nodes.

    Parameters
    ----------
    nodes_xy : (N,2)
        SEM node coordinates (x,z).
    xg, zg : 1D arrays
        Grid coordinates (must be increasing). vp_grid is indexed as vp_grid[iz, ix].
    vp_grid : (nz,nx)
        Background velocity on regular grid.
    method : {'linear','nearest'}
        Interpolation method.
    clip : bool
        If True, clip node coordinates into grid bounds to avoid NaNs outside the domain.

    Returns
    -------
    v_bg : (N,)
        Background velocity at SEM nodes.
    """
    nodes_xy = np.asarray(nodes_xy, dtype=np.float64).reshape(-1, 2)
    xg = np.asarray(xg, dtype=np.float64).reshape(-1)
    zg = np.asarray(zg, dtype=np.float64).reshape(-1)
    vp_grid = np.asarray(vp_grid, dtype=np.float64)

    if vp_grid.shape != (zg.size, xg.size):
        raise ValueError(f"vp_grid shape {vp_grid.shape} must be (len(zg), len(xg)) = {(zg.size, xg.size)}")

    x = nodes_xy[:, 0].copy()
    z = nodes_xy[:, 1].copy()

    if clip:
        x = np.clip(x, xg[0], xg[-1])
        z = np.clip(z, zg[0], zg[-1])

    # RegularGridInterpolator expects points in the order of grid axes: (z, x)
    rgi = RegularGridInterpolator(
        (zg, xg), vp_grid,
        method=str(method),
        bounds_error=False,
        fill_value=None  # allow extrapolation when clip=False; will be linear extrap if possible
    )

    pts = np.column_stack([z, x])
    v_bg = rgi(pts).astype(np.float64)

    # If clip=False and some points are outside, fill_value=None may still yield NaNs (depending on SciPy version).
    # For safety, fall back to nearest for NaNs.
    if np.any(~np.isfinite(v_bg)):
        rgi_near = RegularGridInterpolator((zg, xg), vp_grid, method="nearest", bounds_error=False, fill_value=None)
        v_bg2 = rgi_near(pts).astype(np.float64)
        v_bg = np.where(np.isfinite(v_bg), v_bg, v_bg2)

    return v_bg


def build_velocity_2d_background_with_anomaly(
    nodes_xy: np.ndarray,
    v_background: np.ndarray,
    ctrl6_xy: np.ndarray = None,
    v_inside: float = None,
    tau: float = 10.0,
    samples: int = 800,
    newton_steps: int = 7,
    blend: str = "smooth"   # "smooth" (sigmoid) or "replace"
):
    """
    Compose a 2D background (already interpolated onto SEM nodes) with an optional closed B-spline anomaly.

    - If ctrl6_xy is None: returns pure background.
    - If ctrl6_xy is provided:
        * blend='smooth' : v = v_bg + (v_inside - v_bg) * sigmoid(-sd/tau)
        * blend='replace': set v=v_inside for sd<0 (hard inside)

    Returns
    -------
    v : (N,)
        Final velocity field.
    sd : (N,) or None
        Signed distance to the spline (if anomaly provided), else None.
    extras : dict or None
        Closest points / tangents / normals (if anomaly provided), else None.
    """
    nodes_xy = np.asarray(nodes_xy, dtype=np.float64).reshape(-1, 2)
    v_bg = np.asarray(v_background, dtype=np.float64).reshape(-1)
    if v_bg.size != nodes_xy.shape[0]:
        raise ValueError(f"v_background must have length {nodes_xy.shape[0]} (npoints); got {v_bg.size}")

    if ctrl6_xy is None:
        return v_bg.copy(), None, None

    sd, u_star, Cclosest, t_hat, n_out = signed_distance_to_spline_newton(
        coords_xy=nodes_xy,
        ctrl6_xy=np.asarray(ctrl6_xy, dtype=np.float64).reshape(-1, 2),
        samples=samples,
        newton_steps=newton_steps,
        topk=1,
        return_extra=True
    )

    if blend == "replace":
        v = v_bg.copy()
        inside = sd < 0.0
        v[inside] = float(v_inside)
    else:
        tau_safe = max(float(tau), 1e-12)
        x = np.clip(sd / tau_safe, -50.0, 50.0)
        sig = 1.0 / (1.0 + np.exp(x))  # σ(-sd/τ)
        v = v_bg + (float(v_inside) - v_bg) * sig

    extras = {'u_star': u_star, 'closest': Cclosest, 'tangent': t_hat, 'normal_out': n_out}
    return v, sd, extras
