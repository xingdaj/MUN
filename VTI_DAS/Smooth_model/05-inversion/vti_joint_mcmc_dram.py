...
def pack_params(geo: Geometry, model: VTIModel, invert_depths: bool, fix_last_layer: bool) -> np.ndarray:
...
def unpack_params(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
                  invert_depths: bool, fix_last_layer: bool) -> Tuple[Geometry, VTIModel]:
...
def proposal_std_vector(model: VTIModel, geo: Geometry, prop: Proposal,
                        invert_depths: bool, fix_last_layer: bool) -> np.ndarray:
    layers = active_layer_indices(model, fix_last_layer)
    std = []
    if invert_depths:
        depth_idx = active_depth_indices(model)
...
def prior_ok(geo: Geometry, model: VTIModel, initial_model: VTIModel, prior: Prior,
             invert_depths: bool, fix_last_layer: bool) -> bool:
...
def prior_violation_messages(geo: Geometry, model: VTIModel, initial_model: VTIModel, prior: Prior,
                             invert_depths: bool, fix_last_layer: bool) -> list[str]:
...
def print_initial_objective_diagnostics(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
    ...
    geo, model = unpack_params(theta, template_geo, template_model, invert_depths, fix_last_layer)

    msgs = prior_violation_messages(geo, model, initial_model, prior, invert_depths, fix_last_layer)
    if msgs:
...
def difference_matrix(nr: int, mode: str) -> np.ndarray:
    ...
    mode="adjacent":  d_i = t_i - t_{i+1}, i=0,...,nr-2.
        This is the adjacent-receiver convention used in the manuscript.
    mode="reference": d_i = t_i - t_0, i=1,...,nr-1.

    Both choices produce nr-1 origin-time-free differences.  When the noise is
    added to absolute arrival times, the differential errors are correlated and
    are handled by C_d = sigma^2 A A^T.
    """
    nr = int(nr)
    if nr < 2:
...
def misfit_from_times(pred, obs: ObsData, like: LikelihoodConfig) -> float:
    ...
    For differential P arrivals, d = A t and C_d = sigma^2 A A^T.
    ...
    if like.objective_type in {"diff-p-adjacent", "diff-p-reference"}:
        ...
        pred_d = apply_difference(tp, like.diff_matrix)
        r = pred_d - like.obs_d
        return quadratic_form_inverse(r, like.cov_inv)
...
def objective(theta: np.ndarray, template_geo: Geometry, template_model: VTIModel,
    ...
    geo, model = unpack_params(theta, template_geo, template_model, invert_depths, fix_last_layer)
    if not prior_ok(geo, model, initial_model, prior, invert_depths, fix_last_layer):
...
def run_mcmc(input_dir: Path, output_dir: Path, iterations: int, burnin: int,
    ...
    print("[VERSION] vti_joint_mcmc_dram.py with INIT-CHECK + control.dat qx settings + qx residual tolerance, generated 2026-06-08", flush=True)
    ...
    theta = pack_params(geo0, model0, invert_depths=invert_depths, fix_last_layer=fix_last_layer)
    base_std = proposal_std_vector(model0, geo0, prop, invert_depths=invert_depths, fix_last_layer=fix_last_layer)
    scalar_std = base_std.copy()
    ndim = theta.size
    names = param_names(model0, geo0, invert_depths, fix_last_layer)
    ...
    mis, pred = objective(theta, geo0, model0, model0, prior, obs, invert_depths, fix_last_layer,
                          qx_stop, qx_max_iter, like)
    if not math.isfinite(mis):
        print_initial_objective_diagnostics(
            theta, geo0, model0, model0, prior, obs,
            invert_depths, fix_last_layer, qx_stop, qx_max_iter, like,
        )
    ...
    mean_geo, mean_model = unpack_params(mean_theta, geo0, model0, invert_depths, fix_last_layer)
    best_geo, best_model = unpack_params(best_theta, geo0, model0, invert_depths, fix_last_layer)
...
def param_names(model: VTIModel, geo: Geometry, invert_depths: bool, fix_last_layer: bool) -> list[str]:
    layers = active_layer_indices(model, fix_last_layer)
    names = []
    if invert_depths:
        depth_idx = active_depth_indices(model)
...
def parse_args(argv=None):
    ...
    p.add_argument(
        ...
        help="Likelihood target. diff-p-adjacent uses qP t[i]-t[i+1], matching the manuscript; diff-p-reference uses qP t[i+1]-t[0]. Both use the induced non-diagonal covariance.",
    )
