...
velocity_figures/
    P.png, S.png, E.png, G.png, D.png
        Field/Input / Initial / Mean / Best step profiles.
    ...
    velocity_model_comparison.csv
        Field/Input / Initial / Mean / Best model values.
...
def infer_project_root(user_root: Path | None = None) -> Path:
    ...
    This lets the script run from either the project root or from inside 03-output.
    """
    ...
    for c in unique_candidates:
        if (c / '01-initial').is_dir() and (c / '02-inversion').is_dir() and (c / '03-output').is_dir():
            return c
...
def main(argv=None):
    ...
    p.add_argument('--root', type=Path, default=None, help='Project root containing 01-initial, 02-inversion, 03-output')
    p.add_argument('--true-vel', type=Path, default=None, help='Default uses 01-initial/output/vel.dat. Pass vel1.dat explicitly only for Test 4 diagnostics.')
    ...
    true_vel_path = args.true_vel.expanduser().resolve() if args.true_vel is not None else find_existing(root / '01-initial/output/vel.dat', root / '01-initial/output/vel1.dat')
    init_vel_path = args.init_vel.expanduser().resolve() if args.init_vel is not None else root / '01-initial/output/vel.dat'
    ...
    models = [('Field/Input', m_true), ('Initial', m_init), ('Mean', m_mean), ('Best', m_best)]
    ...
    for k in range(n):
        ...
        for label, model in models:
            ...
            row[f'{safe_label}_dep'] = model.dep[k] if k < len(model.dep) else np.nan
            for par, _, _ in PARAMS:
                row[f'{safe_label}_{par}'] = getattr(model, par)[k] if k < len(getattr(model, par)) else np.nan
        rows.append(row)
