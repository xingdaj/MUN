...
class SEMSimulation:
 ...
 #print("==== Velocity Parameters ====")
    ...
    def _clamped_basis_values_collapsed(self, ctrl_points, u, degree=3, knots=None, ctrl_closed=None):
        """Basis matrix for a closed B-spline with an arbitrary number of control points.

        The convention matches velocity.py: the first free control point is replicated
        `degree` times at the end of the control-polygon to form `ctrl_closed`.
        We then collapse the duplicated controls back onto the first free control.

        Returns
        -------
        B : (M, K) ndarray
            Basis matrix with one column per free control point.
        knots : ndarray
        ctrl_closed : (K + degree, 2) ndarray
        """
        ...
        if ctrl_closed is None or knots is None:
            ctrl_closed = np.vstack([P, np.tile(P[0], (k, 1))])
            n = len(ctrl_closed) - 1
            total_knots = n + k + 2
            knots = np.zeros(total_knots, dtype=np.float64)
            knots[:k+1] = 0.0
        ...
        # Collapse duplicated controls (indices >= n_ctrl all correspond to P0)
        B = np.zeros((M, n_ctrl), dtype=np.float64)
        if n_ctrl >= 1:
            B[:, 0] = B_closed[:, 0]
            if n_closed > n_ctrl:
                B[:, 0] += B_closed[:, n_ctrl:].sum(axis=1)
        if n_ctrl > 1:
            B[:, 1:n_ctrl] = B_closed[:, 1:n_ctrl]
    ...
    def _eval_closed_bspline_and_tangent(self, ctrl_points, t_vals, degree=3):
        """Evaluate closed B-spline curve and unit tangent for arbitrary K control points."""
        P = np.asarray(ctrl_points, dtype=np.float64).reshape(-1, 2)
        k = int(degree)
        ...
        # Map normalized t_vals in [0,1] to the actual parameter interval
