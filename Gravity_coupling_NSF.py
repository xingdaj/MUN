...
# Generate synthetic gravity data using the forward model
...
class PiecewiseRationalQuadraticCoupling(nn.Module):
    def __init__(self, mask, transform_net_create_fn, num_bins=8, tails=None, tail_bound=3.0):
        super().__init__()
        ...
        # Create the transform net
        self.transform_net = transform_net_create_fn(
            (mask <= 0).sum().item(),
            (mask > 0).sum().item() * self._transform_dim_multiplier()
        )
    ...
    def forward(self, inputs):
        # Split inputs
        identity_features = inputs[:, self.mask <= 0]
        transform_features = inputs[:, self.mask > 0]
        
        # Get transform parameters
        transform_params = self.transform_net(identity_features)
        
        # Apply the transform
        transformed, logabsdet = self._coupling_transform(
            transform_features, transform_params, inverse=False
        )
        
        # Combine outputs
        outputs = torch.empty_like(inputs)
        outputs[:, self.mask <= 0] = identity_features
        outputs[:, self.mask > 0] = transformed
    ...
    def inverse(self, inputs):
        ...
        transformed, logabsdet = self._coupling_transform(
            transform_features, transform_params, inverse=True
        )
    ...
    def _coupling_transform(self, inputs, transform_params, inverse=False):
        ...
        if not inverse:
            transformed = inputs * (1 + widths.mean(-1, keepdim=True))
            logabsdet = torch.log(1 + widths.mean(-1, keepdim=True))
        else:
            transformed = inputs / (1 + widths.mean(-1, keepdim=True))
            logabsdet = -torch.log(1 + widths.mean(-1, keepdim=True))
...
class NormalizingFlow(nn.Module):
    """Normalizing flow using neural spline flow (NSF) coupling transformations"""
    def __init__(self, dim=3, num_flows=8):
        ...
        # Create NSF coupling flow layers
        self.flows = nn.ModuleList()
        for i in range(num_flows):
            # Create mask (alternating pattern)
            mask = torch.ones(dim)
            mask[::2] = 0
            if i % 2 == 1:  # Reverse mask every other layer
                mask = 1 - mask
            
            # Define transform net creation function
            ...
            coupling_layer = PiecewiseRationalQuadraticCoupling(
                mask=mask,
