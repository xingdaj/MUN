...
# Anomaly velocity (uniform inside the B-spline boundary)
v_inside = 2000.0

# Signed distance and hard inside/outside assignment (no smooth transition for plotting)
px = X.ravel()
py = Z.ravel()
...
ax.scatter(
    ...
    marker="^",
    s=60,                    # Smaller than sources
    facecolor="#1f77b4",     # Matplotlib default blue (clean)
    edgecolor="red",       # White edge for visibility on any background
    linewidth=1.2,
    zorder=8,
    label="Receivers"
)
...
fig.tight_layout()
fig.savefig("velocity_model.png", dpi=600, bbox_inches="tight")
plt.close(fig)

print("Saved: velocity_model_true_uniform.png")
