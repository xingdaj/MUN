...
from .velocity import build_velocity_on_sem_nodes, build_background_from_grid, build_velocity_2d_background_with_anomaly
from .utils import check_cfl_condition, adjust_time_step_for_stability, gll_points, lagrange_basis
...
class SEMSimulation:
    ...
    def _setup_receivers_from_positions(self, receiver_positions,
                                        xmin, zmin, dx_elem, dz_elem,
                                        nelem_x, nelem_z,
                                        global_connectivity, global_coords, npol):
        """Build receiver interpolation for arbitrary receiver positions.

        Parameters
        ----------
        receiver_positions : array-like of shape (nrec, 2)
            List/array of (x, z) receiver coordinates in SEM domain.

        Returns
        -------
        receiver_coords_sem : list of (x, z)
        receiver_interp_info_sem : list of dict with element index / local coords / weights
        R_sparse : scipy.sparse.csr_matrix, shape (nrec, npoints)
        R_torch : torch.sparse_coo_tensor, same shape as R_sparse
        """
        ...
        if receiver_positions.ndim != 2 or receiver_positions.shape[1] != 2:
            raise ValueError("receiver_positions must be of shape (nrec, 2)")
        ...
        for i, (rx, rz) in enumerate(receiver_positions):
            rx = float(rx)
            rz = float(rz)

            # Find the element index (ex, ez) containing this receiver (clamped to domain)
            ...
            # Local (xi, eta) coordinates in [-1, 1]x[-1, 1]
            x0 = xmin + ex * dx_elem
            z0 = zmin + ez * dz_elem
            xi = 2.0 * (rx - x0) / dx_elem - 1.0
            eta = 2.0 * (rz - z0) / dz_elem - 1.0

            # Compute tensor-product Lagrange basis at (xi, eta)
            weights = np.zeros(len(elem_nodes), dtype=np.float64)
            for iz in range(ngll):
                lz = lagrange_basis(eta, iz, gll_nodes)
                for ix in range(ngll):
                    lx = lagrange_basis(xi, ix, gll_nodes)
                    idx_local = iz * ngll + ix
                    weights[idx_local] = lx * lz

            receiver_coords_sem.append((rx, rz))
            receiver_interp_info_sem.append({
                'elem_idx': elem_idx,
                'xi': xi,
                'eta': eta,
                'elem_nodes': np.asarray(elem_nodes, dtype=np.int64),
                'weights': weights.copy()
            })

            for j, node_id in enumerate(elem_nodes):
                w_ij = weights[j]
                if w_ij != 0.0:
                    rows.append(i)
                    cols.append(int(node_id))
                    data.append(w_ij)

        R_sparse = sp.csr_matrix((data, (rows, cols)), shape=(receiver_positions.shape[0], npoints))

        # Use existing helper to build torch sparse matrix
        from .receivers import create_torch_interpolation_matrix
    ...
    def _initialize_components(self):
        ...
        # receivers: support either discrete positions or circular array
        if 'positions' in receiver_params:
            rec_pos = np.asarray(receiver_params['positions'], dtype=np.float64)
            if rec_pos.ndim != 2 or rec_pos.shape[1] != 2:
                raise ValueError("config['receivers']['positions'] must be of shape (nrec, 2)")
            self.receiver_positions = rec_pos
            self.num_receivers = rec_pos.shape[0]
            # No single radius in the discrete case
        ...
        # Allow both legacy keys ('inside_velocity'/'outside_velocity') and
        # newer generic keys ('vmin'/'vmax') from the main script config.
        vmin_cfg = velocity_params.get('vmin', 2000.0)
        vmax_cfg = velocity_params.get('vmax', 3000.0)
        self.vmin = float(vmin_cfg)
        self.vmax = float(vmax_cfg)

        # ---- Anomaly configuration (may be disabled for 'true model' runs) ----
        anom_cfg = velocity_params.get('anomaly', {}) if isinstance(velocity_params.get('anomaly', {}), dict) else {}
        self.anomaly_enabled = bool(anom_cfg.get('enabled', True))

        # Prefer anomaly.* fields when present (keeps backwards-compat with legacy keys)
        _ctrl = anom_cfg.get('control_points', velocity_params.get('control_points', None))
        self.ctrl_pts_original = None if _ctrl is None else np.asarray(_ctrl, dtype=np.float64)

        _perts = anom_cfg.get('perturbations', velocity_params.get('perturbations', None))
        if _perts is None:
            self.true_offset = None
        else:
            _a = np.asarray(_perts, dtype=np.float64)
            # Treat empty list / empty array as "no offsets"
            self.true_offset = None if _a.size == 0 else _a.reshape(-1, 2)
 ...
 #print("==== Velocity Parameters ====")
        ...
        # -------- 3.5) load 2D smooth background (required) --------
        bg_cfg = velocity_params.get('background_2d', None)
        if bg_cfg is None:
            # allow a shorter alias
            bg_cfg = velocity_params.get('background', None)
        if bg_cfg is None:
            raise ValueError(
                "velocity.background_2d is required for 2D background mode. "
                "Provide {'vp_path': 'vp_smooth.npy', 'x': ..., 'z': ...} or metadata."
            )

        # --- load vp grid ---
        if 'vp' in bg_cfg and bg_cfg['vp'] is not None:
            vp_grid = np.asarray(bg_cfg['vp'], dtype=np.float64)
        else:
            vp_path = bg_cfg.get('vp_path', None)
            if vp_path is None:
                raise ValueError("background_2d must provide either 'vp' array or 'vp_path'.")
            vp_grid = np.load(vp_path).astype(np.float64)

        # --- load x/z grid coordinates ---
        # Preferred: explicit 1D arrays 'x' and 'z'
        if 'x' in bg_cfg and bg_cfg['x'] is not None:
            xg = np.asarray(bg_cfg['x'], dtype=np.float64).reshape(-1)
        elif 'x_path' in bg_cfg and bg_cfg['x_path'] is not None:
            xg = np.load(bg_cfg['x_path']).astype(np.float64).reshape(-1)
        else:
            # Fallback: metadata
            xmin_bg = float(bg_cfg.get('xmin', self.xmin))
            xmax_bg = float(bg_cfg.get('xmax', self.xmax))
            nx_bg   = int(bg_cfg.get('nx', vp_grid.shape[1]))
            xg = np.linspace(xmin_bg, xmax_bg, nx_bg, dtype=np.float64)

        if 'z' in bg_cfg and bg_cfg['z'] is not None:
            zg = np.asarray(bg_cfg['z'], dtype=np.float64).reshape(-1)
        elif 'z_path' in bg_cfg and bg_cfg['z_path'] is not None:
            zg = np.load(bg_cfg['z_path']).astype(np.float64).reshape(-1)
        else:
            zmin_bg = float(bg_cfg.get('zmin', self.zmin))
            zmax_bg = float(bg_cfg.get('zmax', self.zmax))
            nz_bg   = int(bg_cfg.get('nz', vp_grid.shape[0]))
            zg = np.linspace(zmin_bg, zmax_bg, nz_bg, dtype=np.float64)

        # Ensure xg/zg are increasing (required by interpolator)
        if xg.size < 2 or zg.size < 2:
            raise ValueError("background_2d grid x/z must have at least 2 points each.")
        if xg[0] > xg[-1]:
            xg = xg[::-1]
            vp_grid = vp_grid[:, ::-1]
        if zg[0] > zg[-1]:
        ...
        # Background on SEM nodes (cached for forward + adjoint projection)
        interp_method = str(bg_cfg.get('interp', 'linear'))
        self.background_2d = {'x': xg, 'z': zg, 'vp': vp_grid, 'interp': interp_method}
        self.v_background = build_background_from_grid(
        ...
        # Update vmin/vmax for CFL safety (in case config bounds are loose)
        self.vmin = float(min(self.vmin, np.nanmin(self.v_background)))
        self.vmax = float(max(self.vmax, np.nanmax(self.v_background)))
        ...
        # Build an initial velocity model for CFL/time-step checks.
        # If anomaly injection is disabled (e.g., true model comes directly from vp_smooth),
        # we must NOT touch any B-spline control points/offsets.
        if (not getattr(self, 'anomaly_enabled', True)) or (self.ctrl_pts_original is None):
            velocity_model_init = self.v_background.copy()
            signed_dist_init = None
        else:
            # If offsets are not provided, use zero offsets (i.e., the base control points)
            if self.true_offset is None:
                ctrl6 = self.ctrl_pts_original.reshape(-1, 2)
            else:
                # Defensive reshape (supports flat 2K vectors or (K,2))
                _off = np.asarray(self.true_offset, dtype=np.float64)
                _off = _off.reshape(-1, 2)
                ctrl6 = (self.ctrl_pts_original.reshape(-1, 2) + _off)

            tau_cfg = float(self.config['velocity']['tau'])
            spline_samples = int(self.config['velocity']['spline_samples'])

            velocity_model_init, signed_dist_init, _extras = build_velocity_2d_background_with_anomaly(
                nodes_xy=self.global_coords,
                v_background=self.v_background,
                ctrl6_xy=ctrl6,
                v_inside=float(self.config['velocity'].get('anomaly', {}).get('v_inside', self.vmin)),
                tau=tau_cfg,
                samples=spline_samples,
                newton_steps=7,
                blend=str(self.config['velocity'].get('anomaly', {}).get('blend', 'smooth')),
            )
        ...
        if getattr(self, 'receiver_positions', None) is not None:
            # Discrete receiver positions provided in config
            self.receiver_coords_sem, self.receiver_interp_info_sem, self.R_sparse, self.R_torch = \
                self._setup_receivers_from_positions(
            ...
            # Fallback to circular receiver array
    ...
    def run(self, ctrl_offsets_flat=None, use_torch=True):
        ...
        # In this workflow:
        #   - Observation (true) run: anomaly is disabled; vp_smooth already contains the CO2 region.
        #   - Inversion run: anomaly is enabled; control points are optimized by the NF.
        if (not getattr(self, 'anomaly_enabled', True)) or (self.ctrl_pts_original is None):
            # Pure 2D background model (no B-spline injection)
            self.velocity_model = self.v_background.copy()
            self.signed_dist = None
            self.spline_extras = None
        else:
            # If offsets are not provided, use:
            #   (a) the stored true_offset if present, else
            #   (b) zeros (i.e., base control points)
            if ctrl_offsets_flat is None:
                if self.true_offset is not None:
                    ctrl_offsets_flat = torch.from_numpy(np.asarray(self.true_offset, dtype=np.float64).reshape(-1).copy()).double()
                else:
                    ctrl_offsets_flat = np.zeros((self.ctrl_pts_original.reshape(-1, 2).shape[0] * 2,), dtype=np.float64)

            # offset(2K,) -> absolute control points(K,2)
            if isinstance(ctrl_offsets_flat, torch.Tensor):
                perts_np = ctrl_offsets_flat.detach().cpu().numpy().reshape(-1, 2)
            else:
                perts_np = np.asarray(ctrl_offsets_flat, dtype=np.float64).reshape(-1, 2)

            ctrl6_abs = self.ctrl_pts_original.reshape(-1, 2) + perts_np  # absolute control points
            ...
            anom_cfg = vel_cfg.get('anomaly', {})
            v_inside = float(anom_cfg.get('v_inside', float(self.vmin)))
            blend = str(anom_cfg.get('blend', 'smooth'))

            self.velocity_model, self.signed_dist, self.spline_extras = build_velocity_2d_background_with_anomaly(
                nodes_xy=self.global_coords,          # (N,2)
                v_background=self.v_background,       # (N,)
                ctrl6_xy=ctrl6_abs,                   # (K,2)
                v_inside=v_inside,
                tau=tau,
                samples=samples,
                newton_steps=7,
    ...
    def _compute_spline_and_derivatives(self, control_points, t_samples, basis_derivs, n_ctrl):
        ...
        for i, t in enumerate(t_samples):
            s_x, s_z = 0.0, 0.0
            for j in range(n_ctrl + 1):
                basis_val = self.bspline_basis(t, j, 2, self._create_periodic_knots(n_ctrl, 2))
    ...
    def _eval_dC_dctrl(self, ctrl_points, t_samples, degree=2):
        """Compute ∂C/∂ctrl_j using B-spline basis values for a closed curve.

        This version supports an arbitrary number of control points K (not fixed to 6).
        """
        ctrl_points = np.asarray(ctrl_points, dtype=np.float64).reshape(-1, 2)
        n_ctrl = ctrl_points.shape[0]

        t_samples = np.asarray(t_samples, dtype=np.float64).reshape(-1)
        S = t_samples.shape[0]

        # Construct closed control polygon and clamped knot vector
        k = int(degree)
        ...
        # Use basis function values (not derivatives) so that
        #   ∂C/∂ctrl_{j,x} = N_j,  ∂C/∂ctrl_{j,z} = N_j
        for i, t in enumerate(t_samples):
            for j in range(n_ctrl):
                Nj = self.bspline_basis(t, j, degree, knots)
                dC[i, j, 0] = Nj
                dC[i, j, 1] = Nj

        return dC

    def _clamped_basis_values_collapsed(self, ctrl_points, u, degree=2, ctrl_closed=None, knots=None):
        """
        Basis matrix for the SAME closed periodic B-spline convention used by
        sem_waveform.velocity and the model-generation scripts.

        The previous implementation used a clamped [0,1] knot vector and
        collapsed all duplicated controls onto P0.  That was inconsistent with
        the displayed/model-generation boundary.  Here the duplicated controls
        P0..P{k-1} are appended once, and their basis columns are collapsed back
        onto the corresponding free controls.

        Parameters
        ----------
        ctrl_points : (K,2)
        u : (M,)
            normalized periodic parameters in [0,1), as returned by
            signed_distance_to_spline_newton extras['u_star'].
        """
        P = np.asarray(ctrl_points, dtype=np.float64).reshape(-1, 2)
        n_ctrl = P.shape[0]
        k = int(degree)
        if n_ctrl < k + 1:
            raise ValueError(f"At least {k + 1} control points are required for degree {k}; got {n_ctrl}")

        if ctrl_closed is None or knots is None:
            ctrl_closed = np.vstack([P, P[:k]])
            knots = np.arange(0.0, len(ctrl_closed) + k + 1, dtype=np.float64)

        u = np.asarray(u, dtype=np.float64).reshape(-1)
        u_min = float(knots[k])
        u_max = float(knots[-(k + 1)])
        u_norm = np.mod(u, 1.0)
        t_hat = u_min + u_norm * (u_max - u_min)
        t_hat = np.clip(t_hat, u_min, np.nextafter(u_max, u_min))

        M = u.shape[0]
        n_closed = ctrl_closed.shape[0]
        B_closed = np.zeros((M, n_closed), dtype=np.float64)
        for j in range(n_closed):
            coeff = np.zeros(n_closed, dtype=np.float64)
            coeff[j] = 1.0
            sp = BSpline(knots, coeff, k, extrapolate=False)
            B_closed[:, j] = sp(t_hat)

        B = np.zeros((M, n_ctrl), dtype=np.float64)
        B[:, :n_ctrl] = B_closed[:, :n_ctrl]
        # Appended controls are P0, P1, ..., P{k-1}; collapse each onto itself.
        for j in range(n_closed - n_ctrl):
            B[:, j] += B_closed[:, n_ctrl + j]

        return B, knots, ctrl_closed
    ...
    def _analytical_gradient_projection_improved(
        ...
        """Analytical projection dL/dv -> dL/dctrl for arbitrary K control points."""
        vel_cfg = self.config['velocity']

        ctrl = np.asarray(bspline_ctrl, dtype=np.float64).reshape(-1, 2)
        n_ctrl = ctrl.shape[0]

        # vmin/vmax were already validated in _initialize_components as self.vmin/self.vmax,
        # but we read from config with the same flexible key logic for safety.
        vmin_cfg = vel_cfg.get('vmin', self.vmin)
        vmax_cfg = vel_cfg.get('vmax', self.vmax)
        vmin = float(vmin_cfg)
        vmax = float(vmax_cfg)

        # Layered background parameters (for FD consistency)

        anom_cfg = vel_cfg.get('anomaly', {})
        v_inside = float(anom_cfg.get('v_inside', vmin))
        blend = str(anom_cfg.get('blend', 'smooth'))
        ...
        # 2D background: v = v_bg + (v_inside - v_bg) * sigmoid(-sd/tau)
        v_bg = getattr(self, 'v_background', None)
        if v_bg is None:
            # reconstruct from cached background grid (should not happen in normal usage)
            bg = getattr(self, 'background_2d', None)
            if bg is None:
                raise RuntimeError("2D background was not initialized (missing self.v_background).")
            v_bg = build_background_from_grid(
                self.global_coords, bg['x'], bg['z'], bg['vp'],
                method=str(bg.get('interp', 'linear')), clip=True
            )
        v_bg = np.asarray(v_bg, dtype=np.float64).reshape(-1)

        dv_ddsoft = (v_inside - v_bg) * sig * (1.0 - sig) * (-1.0 / tau_safe)

        # Keep only effective contributions
        mask = np.abs(dv_ddsoft) > 1e-8 * np.maximum(1.0, np.abs(v_inside - v_bg))
        if not np.any(mask):
            return {
                'grad_ctrl_x': np.zeros(n_ctrl, dtype=np.float64),
                'grad_ctrl_z': np.zeros(n_ctrl, dtype=np.float64),
            }
        ...
        # Basis matrix for free control points
        B, _, _ = self._clamped_basis_values_collapsed(ctrl, u_hat, degree=2)

        nx = n_out[:, 0:1]
        nz = n_out[:, 1:2]
        grad_x = np.sum(w_node * (B * nx), axis=0)
        grad_z = np.sum(w_node * (B * nz), axis=0)

        return {'grad_ctrl_x': grad_x, 'grad_ctrl_z': grad_z}

    def _eval_closed_bspline_and_tangent(self, ctrl_points, t_vals, degree=2):
        """Evaluate the closed periodic B-spline and unit tangent."""
        P = np.asarray(ctrl_points, dtype=np.float64).reshape(-1, 2)
        k = int(degree)
        if P.shape[0] < k + 1:
            raise ValueError(f"At least {k + 1} control points are required for degree {k}; got {P.shape[0]}")
        ...
        t_norm = np.mod(t_vals, 1.0)
        t_hat = u_min + t_norm * (u_max - u_min)
        t_hat = np.clip(t_hat, u_min, np.nextafter(u_max, u_min))
        ...
        C = np.stack([spl_x(t_hat), spl_z(t_hat)], axis=-1)
        T = np.stack([dspl_x(t_hat), dspl_z(t_hat)], axis=-1)
    ...
    def _finite_difference_gradient_projection(self, perturbations, dJ_dv,
        ...
        """Finite-difference check of projection dJ/dv -> dJ/dctrl for arbitrary K control points."""
        vel_cfg = self.config['velocity']
        # vmin/vmax were already validated in _initialize_components as self.vmin/self.vmax,
        # but we read from config with the same flexible key logic for safety.
        vmin_cfg = vel_cfg.get('vmin', self.vmin)
        ...
        ctrl_base = np.asarray(perturbations, dtype=np.float64).reshape(-1, 2)
        n_ctrl = ctrl_base.shape[0]
        n_param = 2 * n_ctrl
        dJ_dctrl = np.zeros(n_param, dtype=np.float64)

        for j in range(n_param):
            dp = np.zeros_like(ctrl_base)
            if j < n_ctrl:
                dp[j, 0] = fd_eps
            else:
                dp[j - n_ctrl, 1] = fd_eps

            ctrl_plus = ctrl_base + dp
            ctrl_minus = ctrl_base - dp
            v_plus, _, _ = build_velocity_2d_background_with_anomaly(
                nodes_xy=self.global_coords,
                v_background=self.v_background,
                ctrl6_xy=ctrl_plus,
                v_inside=v_inside,
            ...
            v_minus, _, _ = build_velocity_2d_background_with_anomaly(
                nodes_xy=self.global_coords,
                v_background=self.v_background,
                ctrl6_xy=ctrl_minus,
                v_inside=v_inside,
    ...
    def run_forward_and_adjoint(self, params_dict):
        ...
        # 0) B-spline control points (allow arbitrary K2)
        perts = np.asarray(params_dict['bspline_ctrl'], dtype=np.float64)
        if perts.ndim == 1:
            if perts.size % 2 != 0:
                raise ValueError(f"bspline_ctrl 1D array length {perts.size} is not even; cannot reshape to (K,2)")
            perts = perts.reshape(-1, 2)      # absolute control points (K,2)
        elif perts.ndim == 2 and perts.shape[1] == 2:
            perts = perts.astype(np.float64, copy=False)
        else:
            raise ValueError(f"bspline_ctrl must be shape (2*K,) or (K,2), got {perts.shape}")
        n_ctrl = perts.shape[0]
        ...
        #vmin = float(vel_cfg.get('vmin', getattr(self, 'vmin', 2000.0)))
        #vmax = float(vel_cfg.get('vmax', getattr(self, 'vmax', 3000.0)))
        # vmin/vmax were already validated in _initialize_components as self.vmin/self.vmax,
        # but we read from config with the same flexible key logic for safety.
        vmin_cfg = vel_cfg.get('vmin', self.vmin)
        vmax_cfg = vel_cfg.get('vmax', self.vmax)
        ...
        ctrl6 = perts.reshape(-1, 2)
        # geometry result for this sample forward, adjoint projection
        # --- Layered background + spline anomaly (boundary unknown, v_inside known) ---

        anom_cfg = vel_cfg.get('anomaly', {})
        v_inside = float(anom_cfg.get('v_inside', vmin))
        blend = str(anom_cfg.get('blend', 'smooth'))
        # 2D background is cached at init as self.v_background

        velocity_model, signed_dist, spline_extras = build_velocity_2d_background_with_anomaly(
            nodes_xy=self.global_coords,
            v_background=self.v_background,
            ctrl6_xy=ctrl6,
            v_inside=v_inside,
        ...
        #self.        # 4) forward (adjoint)
        # Use configurable history dtype to reduce memory if desired
        # --- Misfit time-window + optional decimation (single knob via config['method']['misfit_window']) ---
        mw = self.config.get('method', {}).get('misfit_window', None)
        mw_enabled = bool(mw and mw.get('enabled', False))
        if mw_enabled:
            tmin_val = mw.get('tmin', 0.0)
            tmin = float(tmin_val) if (tmin_val is not None) else 0.0
            tmax = mw.get('tmax', None)
            if tmax is None:
                wl = mw.get('window_len', None)
                if wl is not None:
                    tmax = tmin + float(wl)
            tmax = float(tmax) if (tmax is not None) else float(self.dt * (self.nt - 1))
            if tmax <= tmin:
                raise ValueError(f"misfit_window invalid: tmax({tmax}) <= tmin({tmin})")
            decim = int(mw.get('decim', 1))
            decim = 1 if decim < 1 else decim
            it0 = int(round(tmin / self.dt))
            it1 = int(round(tmax / self.dt)) + 1
            it0 = max(0, min(it0, self.nt - 1))
            it1 = max(it0 + 1, min(it1, self.nt))
            nt_eff = it1 - it0
        else:
            tmin = 0.0
            tmax = float(self.dt * (self.nt - 1))
            decim = 1
            it0 = 0
            it1 = self.nt
            nt_eff = self.nt

        if mw_enabled and (decim > 1):
            dt_eff = self.dt * decim
            nt_dec = (nt_eff + decim - 1) // decim
        else:
            dt_eff = self.dt
            nt_dec = nt_eff
        hist_dtype = torch.float32 if self.adj_history_dtype == 'float32' else torch.float64

        u_prev = torch.zeros(self.npoints, dtype=torch.float64)
        u_curr = torch.zeros(self.npoints, dtype=torch.float64)
        u_next = torch.zeros(self.npoints, dtype=torch.float64)

        # Make history local (not stored on self) so it can be freed after this call
        u_hist = torch.zeros((nt_eff, self.npoints), dtype=hist_dtype)
        ...
        for it in range(self.nt):
            ...
            u_prev, u_curr, u_next = u_curr, u_next, u_prev
            # store history in chosen precision
            if (it >= it0) and (it < it1):
                u_hist[it - it0, :] = u_curr.to(hist_dtype)
        ...
        #else:
        #    d_syn = np.zeros((nt_eff, 0), dtype=np.float64)

        # Synthesize receiver records
        if self.R_torch is not None:
            # Cast history back to float64 for operator application
            d_syn = (self.R_torch @ u_hist.T.to(torch.float64)).T.detach().cpu().numpy()
        else:
            d_syn = np.zeros((self.nt, 0), dtype=np.float64)


                # Data alignment and likelihood (apply same window/decimation as configured)
        # d_syn is already in the window [it0:it1] with native dt=self.dt and length nt_eff.
        if mw_enabled and (decim > 1):
            d_syn_dec = d_syn[::decim, :]
        else:
            d_syn_dec = d_syn

        if (abs(dt_eff - obs_dt) < 1e-12) and (nt_dec == obs_nt) and (getattr(y_obs, 'shape', [None])[0] == nt_dec):
            residual_dec = d_syn_dec - y_obs
        else:
            residual_dec = self._align_observation_data(d_syn_dec, dt_eff, nt_dec) - \
                           self._align_observation_data(y_obs, obs_dt, obs_nt)

        # Full-resolution residual to drive adjoint (zeros at non-decimated steps)
        if mw_enabled and (decim > 1):
            residual_full = np.zeros((nt_eff, residual_dec.shape[1]), dtype=np.float64)
            residual_full[::decim, :] = residual_dec
        else:
            residual_full = residual_dec

        res_norm2 = float(np.sum(residual_dec * residual_dec))
...
# 5) adjoint, node dJ/dv
        ...
        if self.RT_torch is not None:
            residual_t = torch.from_numpy(residual_full.T).to(torch.float64)
        ...
        for it in range(nt_eff - 1, -1, -1):
            ...
            # retrieve forward field from history, cast to float64 for sensitivity
            gu = u_hist[it, :].to(torch.float64)
            gl = lam_curr
            dJ_dv = dJ_dv + accumulate_sensitivity_dJdc_torch(
                gu, gl, conn, Gx_t, Gz_t, S_t, w2D,
                self.velocity_model_torch, self.npol, self.npoints
        ...
        if VERIFY_PROJECTION:
            ...
            # 2) Dimension-wise comparison: projection ADJ vs projection FD
            proj_an = np.concatenate([proj['grad_ctrl_x'], proj['grad_ctrl_z']], axis=0)
            print("\nPer-dimension gradient comparison:")
            print("Dim |     Adjoint     |    Numerical    |    Ratio    |  Relative error")
            print("-----------------------------------------------------------------")
            for i in range(len(proj_an)):
                adj_val = proj_an[i]
                num_val = proj_fd[i]
                if np.abs(num_val) > 0:
            ...
            # 3) Full FD: finite-difference check on log-likelihood (this is slower).
            print('\nPerforming full finite-difference check on log-likelihood (this is slower).')
            fd_eps_full = 1e-6
            n_param = 2 * n_ctrl
            loglik_fd = np.zeros(n_param, dtype=np.float64)

            for k in range(n_param):
                dp = np.zeros_like(perts)
                if k < n_ctrl:
                    dp[k, 0] = fd_eps_full
                else:
                    dp[k - n_ctrl, 1] = fd_eps_full

                # forward with +eps
                pp = (perts + dp).reshape(-1,)
                res_p = self.run_forward_only_for_loglik(pp, sigma2, obs_dt, obs_nt, y_obs)
                # forward with -eps
                pm = (perts - dp).reshape(-1,)
        ...
        # grad_ctrl is currently [x0..x_{K-1}, z0..z_{K-1}]
        gx = np.asarray(proj['grad_ctrl_x'], dtype=np.float64).reshape(-1)
        gz = np.asarray(proj['grad_ctrl_z'], dtype=np.float64).reshape(-1)
        if gx.shape != gz.shape:
            raise ValueError(f"grad_ctrl_x and grad_ctrl_z must have same shape, got {gx.shape} vs {gz.shape}")
        n_ctrl = gx.size
        grad_ctrl_interleaved = np.empty(2 * n_ctrl, dtype=np.float64)
        grad_ctrl_interleaved[0::2] = gx
        grad_ctrl_interleaved[1::2] = gz
    ...
    def run_forward_only_for_loglik(self, ctrl12, sigma2, obs_dt, obs_nt, y_obs):
        ...
        **will not** self.velocity_model / self.signed_dist / self.spline_extras.
        ...
        """
        # 1) 
        perts = np.asarray(ctrl12, dtype=np.float64).reshape(-1, 2)
        ...
        # --- Layered background + spline anomaly (for likelihood eval) ---
        vel_cfg = self.config['velocity']
        ...
        v_local, sd_local, ex_local = build_velocity_2d_background_with_anomaly(
            nodes_xy=self.global_coords,
            v_background=self.v_background,
            ctrl6_xy=perts,
            v_inside=v_inside,
