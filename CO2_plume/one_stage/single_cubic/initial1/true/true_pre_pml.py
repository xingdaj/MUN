...
# -*- coding: utf-8 -*-
"""
Generate the TRUE PML model with the SAME boundary-to-velocity mapping used by
RQS_NF.py / sem_waveform.velocity:

    vp = vp_background + (vp_inside - vp_background) * sigmoid(-signed_distance/tau)

The closed cubic B-spline convention is also identical:
    ctrl_ext = [P0, ..., P{K-1}, P0, P1, P2]
...
K_SPLINE = int(os.environ.get("K_SPLINE", "3"))
...
def build_vp_models():
    ...
    ctrl_pts = make_ellipse_control_points(ANOMALY_CENTER, N_CTRL, ANOMALY_RX, ANOMALY_RZ, ANOMALY_ROTATION_DEG)
    ...
    return {"x_full": x, "z_full": z, "x_phys": x[ix_phys], "z_phys": z[iz_phys],
            ...
            "control_points": ctrl_pts, "bspline_curve": curve_points,
            "anomaly_mask_full": anomaly_mask, "smoothing_band_full": smoothing_band,
            "signed_distance_full": signed_dist}


def anomaly_json(result):
    return {"enabled": True, "type": "closed periodic cubic B-spline signed-distance sigmoid",
...
def save_outputs(result, workdir):
    ...
    np.save(os.path.join(workdir, "vp_true.npy"), result["vp_smooth_phys"])
    np.save(os.path.join(workdir, "vp_true_pml.npy"), result["vp_smooth_full"])
    ...
    obj_phys = dict(base, model_name="vp_true", description="True Vp model generated by the same signed-distance sigmoid mapping as SEM inversion.",
                    array_file="vp_true.npy", grid=common_grid_json(result["x_phys"], result["z_phys"]))
    obj_full = dict(base, model_name="vp_true_pml", description="True Vp model on full PML grid, generated by the same signed-distance sigmoid mapping as SEM inversion.",
                    array_file="vp_true_pml.npy", grid=common_grid_json(result["x_full"], result["z_full"]),
                    physical_subdomain={"xmin": PHYS_XMIN, "xmax": PHYS_XMAX, "zmin": PHYS_ZMIN, "zmax": PHYS_ZMAX})
    save_json(os.path.join(workdir, "vp_true.json"), obj_phys)
    save_json(os.path.join(workdir, "vp_true_pml.json"), obj_full)
...
def main(workdir=None):
    ...
    print("[OK] Saved true-model outputs to", workdir)
