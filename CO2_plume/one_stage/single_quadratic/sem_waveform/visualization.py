...
def plot_sem_velocity_model(velocity_model, global_coords, ctrl_pts_original, true_offset, 
    ...
    ck = 2  # quadratic B-spline degree
    ...
    if true_offset is not None and ctrl_pts_original is not None:
        ...
        # Create and plot B-spline boundary using the same periodic convention as velocity.py
        ctrl_closed_plot = np.vstack([ctrl_pts_plot, ctrl_pts_plot[:ck]])
        knots = np.arange(0.0, len(ctrl_closed_plot) + ck + 1, dtype=float)
        spline_plot = BSpline(knots, ctrl_closed_plot, ck, extrapolate=False)
        t_curve = np.linspace(float(knots[ck]), float(knots[-(ck+1)]), 500, endpoint=False)
        curve_points_plot = spline_plot(t_curve)
        curve_points_plot = np.vstack([curve_points_plot, curve_points_plot[:1]])
