...
SECTION_INFO = {
    ...
    7: dict(rho=2600, vp=3500, vs=2380, name="Injected CO2"),
...
def pad_for_pml_like_smooth(arr: np.ndarray, pad_x: int, pad_z_top: int) -> np.ndarray:
    ...
    out = arr
    if pad_z_top > 0:
        out = np.pad(out, ((0, pad_z_top), (0, 0)), mode="edge")
    if pad_x > 0:
...
def plot_one_pml(prop_pml, title, unit_label, outname,
    ...
    # z axis follows smooth array direction: 4000 ... -300
    zg_pml = np.linspace(zmax_pml, zmin_pml, nzp)
    ...
    # Show 0 near the top, depth increasing downward; include the top PML (-300)
...
# ==========================================================
# NEW: crop study-area data from the PML model and plot it
# ==========================================================
def crop_and_plot_study_area_from_pml(
    ...
    """
    prop_pml_save:
        saved array orientation, i.e. same orientation as vp_smooth_pml.npy after flipud().
        So row 0 corresponds to z=zmin_pml, row end corresponds to z=zmax_pml.
    ...
    zg = np.linspace(zmin_pml, zmax_pml, nz)  # increasing downward in physical depth
    ...
    # save meta
    meta = {
        "source_array": "vp_smooth_pml.npy",
        "crop_window": {
            "xmin": float(x1),
            "xmax": float(x2),
            "zmin": float(z1),
        ...
        "dz": float(zg[1] - zg[0]) if len(zg) > 1 else None,
        "array_layout": "cropped_array[nz, nx], row-0=zmin, row-end=zmax, col-0=xmin, col-end=xmax"
    }
...
def main(workdir=None):
    ...
    np.save(os.path.join(workdir, "vp_smooth.npy"), vp_smooth)
    np.save(os.path.join(workdir, "vs_smooth.npy"), vs_smooth)
    np.save(os.path.join(workdir, "rho_smooth.npy"), rho_smooth)

    # --- Save coordinate metadata for vp_smooth.npy ---
    vp_meta = {
        "xmin": float(xmin), "xmax": float(xmax), "nx": int(nx),
        "zmin": float(zmin), "zmax": float(zmax), "nz": int(nz),
        "dx": float(dx_grid), "dz": float(dz_grid2),
        "array_layout": "vp_smooth[nz, nx] with z increasing from zmin to zmax, x increasing from xmin to xmax",
        "note": "Saved grid corresponds to physical bounds (xmin/xmax/zmin/zmax). Plotting uses Z_plot stretching only for display."
    }
    meta_path = os.path.join(workdir, "vp_smooth_meta.json")
    ...
    print(f"[OK] Saved numpy fields (TAU_LAYER={TAU_LAYER} m, SMOOTH_ITERS={SMOOTH_ITERS}): "
          "vp_smooth.npy / vs_smooth.npy / rho_smooth.npy")

    # =========================
    # PML (full data saved, including extended 300 m boundary)
    # =========================
    PAD_M = float(os.environ.get("PML_PAD_M", "300.0"))
    pad_x = int(np.round(PAD_M / dx_grid))
    pad_z_top = int(np.round(PAD_M / dz_grid2))

    # Build full padded arrays (these ALREADY contain the full 300 m extensions)
    vp_smooth_pml  = pad_for_pml_like_smooth(vp_smooth,  pad_x=pad_x, pad_z_top=pad_z_top)
    vs_smooth_pml  = pad_for_pml_like_smooth(vs_smooth,  pad_x=pad_x, pad_z_top=pad_z_top)
    rho_smooth_pml = pad_for_pml_like_smooth(rho_smooth, pad_x=pad_x, pad_z_top=pad_z_top)
    ...
    nx_pml = int(vp_smooth_pml.shape[1])
    nz_pml = int(vp_smooth_pml.shape[0])

    # ==========================================================
    # SAVE (FIX): keep FULL padded arrays, but flip vertically at save-time
    # to match smooth.npy vertical orientation (Y-axis consistency).
    # This does NOT crop anything; all 300 m extended boundary data are preserved.
    # ==========================================================
    vp_smooth_pml_save  = np.flipud(vp_smooth_pml)
    vs_smooth_pml_save  = np.flipud(vs_smooth_pml)
    rho_smooth_pml_save = np.flipud(rho_smooth_pml)

    np.save(os.path.join(workdir, "vp_smooth_pml.npy"), vp_smooth_pml_save)
    np.save(os.path.join(workdir, "vs_smooth_pml.npy"), vs_smooth_pml_save)
    np.save(os.path.join(workdir, "rho_smooth_pml.npy"), rho_smooth_pml_save)
    ...
    vp_pml_meta = {
        ...
        "array_layout": "vp_smooth_pml[nz_pml, nx_pml] saved after flipud() so it matches vp_smooth.npy vertical orientation",
        "note": "Saved arrays contain FULL PML extensions (left/right/top = 300 m). Flip is applied ONLY at save-time for orientation consistency."
    }
    meta_pml_path = os.path.join(workdir, "vp_smooth_pml_meta.json")
    with open(meta_pml_path, "w") as f:
        json.dump(vp_pml_meta, f, indent=2)
    print(f"[OK] Saved PML-padded models (PAD_M={PAD_M} m): vp_smooth_pml.npy / vs_smooth_pml.npy / rho_smooth_pml.npy")
    print(f"[OK] Saved meta: {meta_pml_path}")

    # PML images (use unflipped arrays for plotting; already consistent with smooth display)
    plot_one_pml(
        vp_smooth_pml,
        "Vp (smoothed, PML-padded)",
        "Vp (m/s)",
        "property_vp_smooth_pml.png",
        xmin_pml, xmax_pml, zmin_pml, zmax_pml,
        xmin0, xmax0, zmin0, zmax0,
        PAD_M, workdir
    )
    plot_one_pml(
        vs_smooth_pml,
        "Vs (smoothed, PML-padded)",
        "Vs (m/s)",
        "property_vs_smooth_pml.png",
        xmin_pml, xmax_pml, zmin_pml, zmax_pml,
        xmin0, xmax0, zmin0, zmax0,
        PAD_M, workdir
    )
    plot_one_pml(
        rho_smooth_pml,
        "Density (smoothed, PML-padded)",
        "Density (kg/m$^3$)",
        "property_density_smooth_pml.png",
        xmin_pml, xmax_pml, zmin_pml, zmax_pml,
        xmin0, xmax0, zmin0, zmax0,
        PAD_M, workdir
    )
    ...
    # ==========================================================
    # NEW: save and plot study area from PML model
    # Research area: X[0,6500], Z[1000,3000]
    # ==========================================================
    crop_and_plot_study_area_from_pml(
        prop_pml_save=vp_smooth_pml_save,
        xmin_pml=xmin_pml,
        xmax_pml=xmax_pml,
        zmin_pml=zmin_pml,
        zmax_pml=zmax_pml,
        ...
        z2=3200.0,
        workdir=workdir,
        npy_name="vp_smooth_pml_study_area.npy",
        meta_name="vp_smooth_pml_study_area_meta.json",
        fig_name="property_vp_smooth_pml_study_area.png",
    ...
    # --------------------
    # Original plotting functions (non-PML) - unchanged
    # --------------------
    ...
    # Smoothed (for inversion)
    plot_one(rho_smooth, "Density (smoothed)", "Density (kg/m$^3$)", "property_density_smooth.png", key="rho")
    plot_one(vp_smooth,  "Vp (smoothed)", "Vp (m/s)", "property_vp_smooth.png", key="vp")
    plot_one(vs_smooth,  "Vs (smoothed)", "Vs (m/s)", "property_vs_smooth.png", key="vs")
